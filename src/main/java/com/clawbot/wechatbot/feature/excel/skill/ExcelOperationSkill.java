...
/** Excel 表格操作技能：生成、增删改行、分析操作（排序/去重/分组汇总/缺失补全）、列聚合查询、知识管理（解析 → 别名解析 → 校验 → 执行四段式）。 */
@Component
public final class ExcelOperationSkill implements SkillExecutor {
    ...
    /** 四段式：解析（文本 → 计划）→ 知识库别名解析 → 校验（计划 + 当前表格状态）→ 执行（按序，失败即停）。 */
